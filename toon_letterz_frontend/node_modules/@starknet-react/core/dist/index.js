...
// src/connectors/mobile.ts
var isMobile = () => {
  const userAgent = typeof window !== "undefined" ? window.navigator?.userAgent : "";
  return /Android|iPhone|iPad|iPod/i.test(userAgent);
};
var withMobileConnector = (connectors) => {
  if (isMobile() && // In the context of a mobile browser, `starknet_braavos` is only injected
...
function StarknetProvider({
  ...
  return /* @__PURE__ */ jsx2(QueryClientProvider, { client: queryClient ?? defaultQueryClient, children: /* @__PURE__ */ jsx2(StarknetContext.Provider, { value: state, children: /* @__PURE__ */ jsx2(AccountProvider, { account, children }) }) });
}
...
function useEstimateFees({
  ...
  const { account } = useAccount();
  const queryKey_ = useMemo10(
    () => queryKey9({ calls, options }),
    [calls, options]
  );
  const enabled = useMemo10(() => Boolean(enabled_ && calls), [enabled_, calls]);
  useInvalidateOnBlock({
    enabled: Boolean(enabled && watch),
    queryKey: queryKey_
  });
